...
Room::Room(unsigned int serialisableId)
...
void Room::AddRoom(Room::JoiningDirections direction, Room::Pointer pRoom)
{
...
{
	assert(option != nullptr);
	m_staticOptions[m_staticOptionsStartKey++] = option;
}
...
void Room::PrintOptions() const
{
	for (unsigned int i = 0; i < m_dynamicOptions.size(); ++i)
	{
		...
		std::cout << i + 1 << ": " << option->GetOuputText() << std::endl << std::endl << std::endl;
	}
	...
	for (StaticOptions::const_iterator iter = m_staticOptions.cbegin(); iter != m_staticOptions.cend(); ++iter)
	{
		...
		std::cout << chosenOption << ": " << option->GetOuputText() << std::endl << std::endl;
	}
}
...
void Room::OnLoad(std::ifstream& file)
{
	...
	if (numDynamicOptions>0)
	{
		for (unsigned int i = 0; i < numDynamicOptions; ++i)
		{
			...
			file >> optionId;
			Option* pOption = dynamic_cast<Option*>(SerialisationManager::GetSingletonPtr()->GetSerialsable(optionId));
			if (pOption)
			{
